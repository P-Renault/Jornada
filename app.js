...
async function deleteJourney(id,dateHint='',timeHint=''){
 const rawId=String(id ?? '').trim();
 let r=rows.find(x=>String(x.id ?? '').trim()===rawId);
 // Fallback: the visible history card also carries fecha/hora_inicio. This
 // protects against a type/serialization mismatch in the client-side rows.
 if(!r && dateHint && timeHint){
   const d=String(dateHint).slice(0,10);
   const t=String(timeHint).slice(0,5);
   r=rows.find(x=>String(x.fecha).slice(0,10)===d && String(x.hora_inicio||'').slice(0,5)===t);
 }
 if(!r){
   alert('No se pudo identificar la jornada seleccionada. Recarga la página e inténtalo nuevamente.');
   return;
 }
 ...
 const button=document.querySelector(`[data-delete-key="${CSS.escape(rawId || (String(r.fecha)+'|'+String(r.hora_inicio)))}"]`);
 if(button)button.disabled=true;
 try{
   let deleteId=String(r.id ?? '').trim();

   // If the local row has no usable id, resolve the real database id directly
   // from the same date + start time shown on the card.
   if(!deleteId && dateHint && timeHint){
     const d=String(dateHint).slice(0,10), t=String(timeHint).slice(0,5);
     const resolved=await withTimeout(db.from('jornadas_trabajo').select('id').eq('fecha',d).eq('hora_inicio',t+':00').order('created_at',{ascending:false}).limit(1).maybeSingle());
     if(resolved.error)throw resolved.error;
     deleteId=String(resolved.data?.id ?? '').trim();
   }
   if(!deleteId)throw new Error('No se pudo resolver el ID real de la jornada en Supabase.');

   const result=await withTimeout(db.from('jornadas_trabajo').delete().eq('id',deleteId));
   if(result.error)throw result.error;

   const check=await withTimeout(db.from('jornadas_trabajo').select('id').eq('id',deleteId).maybeSingle());
   if(check.error)throw check.error;
   if(check.data)throw new Error('Supabase no eliminó el registro. Revisa las políticas RLS de DELETE.');

   if(active&&String(active.id)===deleteId)active=null;
   await load();
...
function renderHistory(){
 ...
 $('history').innerHTML=a.length?a.map(r=>{
   const c=isClosed(r), id=String(r.id ?? ''), date=String(r.fecha).slice(0,10), time=String(r.hora_inicio||'').slice(0,5);
   const key=id || `${date}|${time}`;
   return `<div class="item"><b>${r.fecha} · ${time}</b><p>${c?'Cerrada':'EN CURSO'} · Meta ${money(r.meta_dia)} · Plan ${Number(r.horas_planificadas||0).toFixed(2)} h</p><p>${c?'Neta '+money(r.ganancia_neta)+' · '+hReal(r).toFixed(2)+' h · '+kmReal(r).toFixed(1)+' km':'Jornada pendiente de cierre'}</p><div class="history-actions">${!c?'<button data-finish="'+id+'">Terminar jornada</button>':''}<button class="danger" data-delete-id="'+id+'" data-delete-date="'+date+'" data-delete-time="'+time+'" data-delete-key="'+key.replace(/"/g,'&quot;')+'">Borrar jornada</button></div></div>`
 }).join(''):'<p>Sin jornadas en este mes.</p>';
 document.querySelectorAll('[data-finish]').forEach(b=>b.onclick=()=>{const r=rows.find(x=>String(x.id)===b.dataset.finish);if(r){fill(r);activeMode(r);switchTab('jornada');$('closePanel').scrollIntoView({behavior:'smooth'})}});
 document.querySelectorAll('[data-delete-id]').forEach(b=>b.onclick=()=>deleteJourney(b.getAttribute('data-delete-id'),b.getAttribute('data-delete-date'),b.getAttribute('data-delete-time')));
}
