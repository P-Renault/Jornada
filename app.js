...
async function deleteJourney(id){
 ...
 try{
   const result=await withTimeout(db.from('jornadas_trabajo').delete().eq('id',id).select('id'));
   if(result.error)throw result.error;
   if(!result.data||result.data.length===0)throw new Error('Supabase no eliminó ningún registro. Revisa la política RLS de DELETE.');
   if(active&&String(active.id)===String(id))active=null;
