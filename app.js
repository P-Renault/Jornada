...
function renderPerformanceChart(){
 ...
 for(const s of series){const pts=data.map((d,i)=>({i,v:d[s.key]}));svg+=`<path d="${linePath(pts,x,y)}" class="chart-line ${s.cls}" fill="none"/>`;pts.forEach(p=>{if(p.v==null)return;const d=data[p.i];svg+=`<circle cx="${x(p.i)}" cy="${y(p.v)}" r="4.5" class="chart-point ${s.cls}"><title>${formatShortDate(d.date)} · ${s.label}: ${money(p.v)}</title></circle>`})}
 svg+='</svg><div class="chart-legend">'+series.map(s=>`<span><i class="legend-dot ${s.cls}"></i>${s.label}</span>`).join('')+'</div>';
 root.innerHTML=svg;
}
