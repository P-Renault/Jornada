...
async function deleteJourney(id){
 ...
 if(!Number.isSafeInteger(numericId)) { alert('ID de jornada no válido.'); return; }
 const r=rows.find(x=>Number(x.id)===numericId);
 if(!r)return;
 ...
 const b=document.querySelector(`[data-delete="${CSS.escape(String(id))}"]`);
 if(b)b.disabled=true;
 try{
   // Ejecutamos DELETE sin .select(). Así no dependemos de la representación
   // de filas eliminadas en PostgREST. Luego verificamos directamente en BD.
   const result=await withTimeout(db.from('jornadas_trabajo').delete().eq('id',numericId));
   if(result.error)throw result.error;

   // Confirmación real: el ID ya no debe existir en Supabase.
   const check=await withTimeout(db.from('jornadas_trabajo').select('id').eq('id',numericId).maybeSingle());
   if(check.error)throw check.error;
   if(check.data)throw new Error('Supabase respondió al DELETE, pero el registro todavía existe. Revisa las políticas RLS de DELETE.');

   if(active&&Number(active.id)===numericId)active=null;
   // Recargar desde Supabase para que Historial, Resumen y Calendario
   // queden sincronizados con la base de datos, no solo con la memoria.
   await load();
...
function renderHistory(){const m=$('histMonth').value||today().slice(0,7);const a=rows.filter(r=>String(r.fecha).slice(0,7)===m);$('history').innerHTML=a.length?a.map(r=>{const c=isClosed(r);return `<div class="item"><b>${r.fecha} · ${String(r.hora_inicio||'').slice(0,5)}</b><p>${c?'Cerrada':'EN CURSO'} · Meta ${money(r.meta_dia)} · Plan ${Number(r.horas_planificadas||0).toFixed(2)} h</p><p>${c?'Neta '+money(r.ganancia_neta)+' · '+hReal(r).toFixed(2)+' h · '+kmReal(r).toFixed(1)+' km':'Jornada pendiente de cierre'}</p><div class="history-actions">${!c?'<button data-finish="'+r.id+'">Terminar jornada</button>':''}<button class="danger" data-delete="'+r.id+'">Borrar jornada</button></div></div>`}).join(''):'<p>Sin jornadas en este mes.</p>';document.querySelectorAll('[data-finish]').forEach(b=>b.onclick=()=>{const r=rows.find(x=>String(x.id)===b.dataset.finish);if(r){fill(r);activeMode(r);switchTab('jornada');$('closePanel').scrollIntoView({behavior:'smooth'})}});document.querySelectorAll('[data-delete]').forEach(b=>b.onclick=()=>deleteJourney(b.dataset.delete))}
