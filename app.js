...
async function connect() {
  ...
  try {
    ...
    const checkPromise = db.from('jornadas_trabajo').select('id').limit(1);
    const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('La conexión con Supabase tardó demasiado. Revisa la URL, la clave y la conexión a Internet.')), 12000));
...
function metricKm(r) {
  const stored = Number(r?.km_recorridos);
  if (Number.isFinite(stored) && stored >= 0 && r?.km_recorridos != null) return stored;
  const start = Number(r?.km_inicio);
  const end = Number(r?.km_final);
  return Number.isFinite(start) && Number.isFinite(end) && end >= start ? end - start : 0;
}

function metricHours(r) {
  const stored = Number(r?.horas_trabajadas);
  if (Number.isFinite(stored) && stored > 0 && r?.horas_trabajadas != null) return stored;
  return hoursBetween(String(r?.hora_inicio ?? '').slice(0,5), String(r?.hora_fin ?? '').slice(0,5));
}
...
function historicalRates() {
  ...
  const h = done.reduce((a,r)=>a+metricHours(r),0);
  const tr = done.reduce((a,r)=>a+Number(r.viajes||0),0);
  const km = done.reduce((a,r)=>a+metricKm(r),0);
...
function renderProjection(p = projection()) {
  ...
  if ($('planRate')) $('planRate').textContent = CLP(p.rates.netHora) + '/h';
...
function setActiveMode(r, focusClose = false) {
  ...
  $('closingNote').textContent = 'Completa los datos reales y pulsa “Cerrar día” para guardar el resultado final.';

  if (focusClose) {
    $('closingPanel').scrollIntoView({behavior:'smooth', block:'start'});
    setTimeout(() => $('horaFin').focus(), 250);
  }
}
...
async function finishCurrent() {
  ...
  // Algunas versiones de la tabla tienen columnas calculadas/generadas
  // (por ejemplo km_recorridos). PostgreSQL no permite escribirlas.
  // Intentamos el cierre con los datos editables y, si Supabase informa
  // otra columna generada, la retiramos y reintentamos automáticamente.
  let updatePayload = { ...payload };
  let updateError = null;
  for (let attempt = 0; attempt < 4; attempt++) {
    const result = await db.from('jornadas_trabajo').update(updatePayload).eq('id', id);
    updateError = result.error || null;
    if (!updateError) break;
    const match = String(updateError.message || '').match(/column [\"']([^\"']+)[\"'].*(?:only be updated to DEFAULT|cannot be updated)/i);
    if (!match || !(match[1] in updatePayload)) break;
    delete updatePayload[match[1]];
  }
  if (updateError) return setMsg('workMsg', 'No se pudo cerrar la jornada: ' + updateError.message);

  await refresh();
  let closed = cache.find(r => String(r.id) === String(id));
  if (!closed) {
    // La actualización fue confirmada aunque la lectura pueda tardar.
    closed = { ...current, ...updatePayload, id, estado:'cerrada' };
    cache = [closed, ...cache.filter(r => String(r.id) !== String(id))];
  }
  fillRecord(closed);
...
function renderClosingAnalysis(r) {
  ...
  const real = { net:Number(r.ganancia_neta || 0), gross:Number(r.ganancia_bruta || 0), hours:metricHours(r), trips:Number(r.viajes || 0), km:metricKm(r), fuel:Number(r.combustible || 0), maint:Number(r.mantenimiento || 0), comm:Number(r.comision_app || 0) };
  const rows = [
...
$('historialLista').addEventListener('click', (e) => {
  ...
  if (isActive(r)) {
    setActiveMode(r, true);
    setMsg('workMsg', 'Jornada recuperada desde Historial. Completa el cierre y pulsa “Cerrar día”.');
  } else {
    setClosedEditMode(r);
    setMsg('workMsg', 'Jornada cerrada cargada para edición.');
  }
});
...
function summary(m) {
  const rs = cache.filter(r => r.fecha.startsWith(m) && isClosed(r));
  const sum = (k) => rs.reduce((a,r) => a + Number(r[k] || 0), 0);
  return { rs, h:rs.reduce((a,r)=>a+metricHours(r),0), km:rs.reduce((a,r)=>a+metricKm(r),0), trips:sum('viajes'), gross:sum('ganancia_bruta'), net:sum('ganancia_neta'), fuel:sum('combustible'), maint:sum('mantenimiento'), comm:sum('comision_app'), goals:sum('meta_dia') };
}
...
async function refresh(){
  ...
  if(error){console.error(error);setMsg('configMsg','Conectado, pero no se pudieron leer las jornadas: '+error.message);return;}
  cache=data||[];
  syncJourneyUI();
