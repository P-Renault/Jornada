...
async function finishCurrent(){
 ...
 if(closed){fillRecord(closed);setClosedEditMode(closed);renderDashboard();renderClosingAnalysis(closed);}
 msg('workMsg','Día cerrado y jornada guardada correctamente. El resultado real quedó almacenado en Supabase y el análisis Meta vs. Plan vs. Real está disponible en esta pantalla y en Resumen.');
}

function renderClosingAnalysis(r){
 const s=settings(), goal=Number(r.meta_dia||0), planNet=Number(r.plan_ganancia_neta||0), planGross=Number(r.plan_ganancia_bruta||0), planHours=Number(r.horas_planificadas||0), planTrips=Number(r.plan_viajes||0), planKm=Number(r.plan_km||0), planFuel=Number(r.plan_combustible||0), planMaint=Number(r.plan_mantenimiento||0), planComm=planGross*s.comisionPct/100;
 const realNet=Number(r.ganancia_neta||0),realGross=Number(r.ganancia_bruta||0),realHours=Number(r.horas_trabajadas||0),realTrips=Number(r.viajes||0),realKm=Number(r.km_recorridos||0),realFuel=Number(r.combustible||0),realMaint=Number(r.mantenimiento||0),realComm=Number(r.comision_app||0);
 const rows=[['Meta del día',goal,realNet,'$'],['Ganancia neta',planNet,realNet,'$'],['Ganancia bruta',planGross,realGross,'$'],['Horas',planHours,realHours,'h'],['Viajes',planTrips,realTrips,'viajes'],['Kilómetros',planKm,realKm,'km'],['Combustible',planFuel,realFuel,'$'],['Mantenimiento',planMaint,realMaint,'$'],['Comisión',planComm,realComm,'$']];
 const box=$('closingAnalysis'); if(!box)return; box.innerHTML='<h3>Análisis de la jornada</h3><p class="muted">Comparación entre lo proyectado al iniciar y el resultado registrado al cerrar el día.</p><div class="table-scroll"><table><thead><tr><th>Indicador</th><th>Plan / meta</th><th>Real</th><th>Diferencia</th></tr></thead><tbody>'+rows.map(function(row){const diff=row[2]-row[1],pct=row[1]?diff/row[1]*100:0;const fmt=row[3]==='$'?CLP:((v)=>v.toFixed(2)+' '+row[3]);return '<tr><td>'+row[0]+'</td><td>'+fmt(row[1])+'</td><td>'+fmt(row[2])+'</td><td class="'+(diff>=0?'positive':'negative')+'">'+(diff>=0?'+':'')+fmt(diff)+' ('+pct.toFixed(0)+'%)</td></tr>';}).join('')+'</tbody></table></div><div class="bars"><div class="bar-row"><span>Meta neta</span><div class="bar-track"><div class="bar goal" style="width:'+Math.min(100,goal/Math.max(goal,realNet,planNet,1)*100)+'%"></div></div><b>'+CLP(goal)+'</b></div><div class="bar-row"><span>Plan neto</span><div class="bar-track"><div class="bar plan" style="width:'+Math.min(100,planNet/Math.max(goal,realNet,planNet,1)*100)+'%"></div></div><b>'+CLP(planNet)+'</b></div><div class="bar-row"><span>Real neto</span><div class="bar-track"><div class="bar actual" style="width:'+Math.min(100,realNet/Math.max(goal,realNet,planNet,1)*100)+'%"></div></div><b>'+CLP(realNet)+'</b></div></div>';
}
