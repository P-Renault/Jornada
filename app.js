...
function saveMaint(){const rec={id:`${Date.now()}-${Math.random().toString(36).slice(2,8)}`,fecha:val('maintDate')||today(),km:val('maintKm')===''?null:Number(val('maintKm')),tipo:val('maintType'),costo:Number(val('maintCost'))||0,concepto:val('maintConcept').trim(),proximoKm:Number(val('maintNextKm'))||0,proximaFecha:val('maintNextDate')||null,nota:val('maintNote').trim()||null};const ref=vehicleOdometer();if(rec.costo<0||!rec.concepto){msg('Completa fecha, concepto y costo de la mantención. El km es opcional y, si lo dejas vacío, se guardará sin kilometraje.');return;}if(ref>0&&rec.km!=null&&rec.km>ref){msg(`El km de la mantención (${rec.km}) no puede superar el odómetro actual (${ref}) durante esta validación.`);return;}const list=maintenanceRecords();list.push(rec);putList(MAINT_KEY,list);clearMaintForm();renderMaintenance();msg('Mantención registrada.');}
...
function saveComp(){const rec={id:`${Date.now()}-${Math.random().toString(36).slice(2,8)}`,nombre:val('compName').trim(),marca:val('compBrand').trim(),modelo:val('compModel').trim(),fecha:val('compDate')||today(),km:val('compKm')===''?null:Number(val('compKm')),costo:Number(val('compCost'))||0,vidaKm:Number(val('compLifeKm'))||0,estado:val('compStatus')};const ref=vehicleOdometer();if(!rec.nombre||rec.costo<0){msg('Completa el componente y costo. El km de instalación es opcional y, si lo dejas vacío, se guardará sin kilometraje.');return;}if(ref>0&&rec.km!=null&&rec.km>ref){msg(`El km de instalación (${rec.km}) no puede superar el odómetro actual (${ref}).`);return;}const list=componentRecords();list.push(rec);putList(COMP_KEY,list);clearCompForm();renderComponents();msg('Componente registrado.');}
...
function render(){renderPlan();renderActive();renderFormIdle();renderMetrics();renderHistory();renderSettings();renderVehicle();renderFuel();renderMaintenance();renderComponents();renderDocs();}
...
loadSettingsIntoForm();loadVehicleForm();$('histMonth').value=today().slice(0,7);$('fecha').value=today();if(vehicleOdometer()>0){$('maintKm').value=vehicleOdometer();$('compKm').value=vehicleOdometer();}
