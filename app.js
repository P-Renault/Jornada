...
// ===== S09: Fondo de desgaste (persistencia local durante validación) =====
const FUND_KEY='b20s9_fund_movements';
const NEED_KEY='b20s9_future_needs';
function fundRecords(){return jsonList(FUND_KEY);}
function needRecords(){return jsonList(NEED_KEY);}
function fundBalance(){return fundRecords().reduce((s,r)=>s+(r.tipo==='aporte'?Number(r.monto||0):-Number(r.monto||0)),0);}
function renderFund(){
 const movements=fundRecords().sort((a,b)=>String(b.fecha).localeCompare(String(a.fecha))||String(b.id).localeCompare(String(a.id)));
 const balance=fundBalance();
 const aportes=movements.filter(r=>r.tipo==='aporte').reduce((s,r)=>s+Number(r.monto||0),0);
 const retiros=movements.filter(r=>r.tipo==='retiro').reduce((s,r)=>s+Number(r.monto||0),0);
 const needs=needRecords().filter(r=>!r.cubierta);
 const future=needs.reduce((s,r)=>s+Number(r.costo||0),0);
 const deficit=Math.max(0,future-Math.max(0,balance));
 $('fundSummary').innerHTML=[['Saldo del fondo',money(balance)],['Aportes acumulados',money(aportes)],['Retiros acumulados',money(retiros)],['Necesidades pendientes',needs.length],['Costo futuro pendiente',money(future)],['Déficit de cobertura',money(deficit)]].map(([a,b])=>`<div><span>${a}</span><strong>${b}</strong></div>`).join('');
 $('fundHistory').innerHTML=movements.length?movements.map(r=>`<article class="item"><div class="row"><b>${String(r.fecha).slice(0,10)} · ${r.concepto}</b><button class="danger fund-delete" data-id="${r.id}">Eliminar</button></div><div class="mini-grid"><span>Movimiento <b>${r.tipo==='aporte'?'Aporte':'Retiro'}</b></span><span>Monto <b>${money(r.monto)}</b></span><span>Km <b>${r.km!=null?Number(r.km).toLocaleString('es-CL'):'—'}</b></span></div><div class="muted">${r.nota||'Sin observaciones'}</div></article>`).join(''):'<div class="empty">Sin movimientos registrados.</div>';
 $('needSummary').innerHTML=[['Saldo disponible',money(Math.max(0,balance))],['Costo futuro pendiente',money(future)],['Déficit total',money(deficit)],['Cobertura global',future>0?`${Math.min(100,Math.max(0,balance/future*100)).toFixed(1)}%`:'Sin necesidades']].map(([a,b])=>`<div><span>${a}</span><strong>${b}</strong></div>`).join('');
 $('needHistory').innerHTML=needs.length?needs.map(r=>{const c=Math.min(100,Math.max(0,balance/Math.max(1,Number(r.costo))*100));const d=Math.max(0,Number(r.costo)-Math.max(0,balance));const kmH=Number(r.horizonteKm||0);const jH=Number(r.horizonteJornadas||0);const perKm=kmH>0?d/kmH:0;const perJourney=jH>0?d/jH:0;return `<article class="item"><div class="row"><b>${r.nombre}</b><span>${r.cubierta?'Cubierta':'Pendiente'}</span></div><div class="mini-grid"><span>Costo <b>${money(r.costo)}</b></span><span>Cobertura <b>${c.toFixed(1)}%</b></span><span>Fecha objetivo <b>${r.fecha||'—'}</b></span><span>Km objetivo <b>${r.kmObjetivo?Number(r.kmObjetivo).toLocaleString('es-CL'):'—'}</b></span></div><div class="muted">Déficit: ${money(d)} · Aporte requerido/km: ${perKm?money(perKm):'—'} · Aporte requerido/jornada: ${perJourney?money(perJourney):'—'}</div><div class="row"><button class="secondary need-cover" data-id="${r.id}">${r.cubierta?'Marcar pendiente':'Cubrir con fondo'}</button><button class="danger need-delete" data-id="${r.id}">Eliminar</button></div></article>`}).join(''):'<div class="empty">Sin necesidades futuras registradas.</div>';
 document.querySelectorAll('.fund-delete').forEach(b=>b.onclick=()=>{if(confirm('¿Eliminar este movimiento del fondo?')){putList(FUND_KEY,fundRecords().filter(x=>x.id!==b.dataset.id));renderFund();msg('Movimiento eliminado.');}});
 document.querySelectorAll('.need-delete').forEach(b=>b.onclick=()=>{if(confirm('¿Eliminar esta necesidad futura?')){putList(NEED_KEY,needRecords().filter(x=>x.id!==b.dataset.id));renderFund();msg('Necesidad eliminada.');}});
 document.querySelectorAll('.need-cover').forEach(b=>b.onclick=()=>{const list=needRecords();const i=list.findIndex(x=>x.id===b.dataset.id);if(i<0)return;const bal=fundBalance();const n=list[i];if(!n.cubierta && bal<Number(n.costo)){msg('El fondo disponible no cubre completamente esta necesidad.');return;}list[i].cubierta=!n.cubierta;putList(NEED_KEY,list);renderFund();msg(list[i].cubierta?'Necesidad marcada como cubierta.':'Necesidad marcada como pendiente.');});
}
function clearFundForm(){['fundDate','fundAmount','fundKm','fundConcept','fundNote'].forEach(id=>$(id).value='');$('fundDate').value=today();$('fundType').value='aporte';}
function saveFund(){const fecha=val('fundDate')||today(),tipo=val('fundType'),monto=Number(val('fundAmount')||0),rawKm=val('fundKm'),km=rawKm===''?null:Number(rawKm),concepto=val('fundConcept').trim();const ref=vehicleOdometer();if(!fecha||!['aporte','retiro'].includes(tipo)||!Number.isFinite(monto)||monto<=0||!concepto){msg('Completa fecha, movimiento, monto y concepto.');return;}if(km!==null&&(!Number.isFinite(km)||km<0)){msg('El kilometraje debe ser válido.');return;}if(ref>0&&km!==null&&km>ref){msg(`El km asociado (${km}) no puede superar el odómetro actual (${ref}).`);return;}if(tipo==='retiro'&&monto>Math.max(0,fundBalance())){msg('El retiro no puede superar el saldo disponible del fondo.');return;}const rec={id:`${Date.now()}-${Math.random().toString(36).slice(2,8)}`,fecha,tipo,monto,km,concepto,nota:val('fundNote').trim()||null};const list=fundRecords();list.push(rec);putList(FUND_KEY,list);clearFundForm();renderFund();msg('Movimiento del fondo registrado.');}
function clearNeedForm(){['needName','needAmount','needDate','needKm','needHorizonKm','needHorizonJourneys'].forEach(id=>$(id).value='');}
function saveNeed(){const nombre=val('needName').trim(),costo=Number(val('needAmount')||0),fecha=val('needDate')||null,km=val('needKm')===''?null:Number(val('needKm')),hkm=Number(val('needHorizonKm')||0),hj=Number(val('needHorizonJourneys')||0);if(!nombre||!Number.isFinite(costo)||costo<=0){msg('Completa necesidad y costo estimado.');return;}if(km!==null&&(!Number.isFinite(km)||km<0)){msg('El km objetivo debe ser válido.');return;}if(hkm<0||hj<0){msg('Los horizontes no pueden ser negativos.');return;}const rec={id:`${Date.now()}-${Math.random().toString(36).slice(2,8)}`,nombre,costo,fecha,kmObjetivo:km,horizonteKm:hkm,horizonteJornadas:hj,cubierta:false};const list=needRecords();list.push(rec);putList(NEED_KEY,list);clearNeedForm();renderFund();msg('Necesidad futura registrada.');}
