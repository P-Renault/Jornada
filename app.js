...
async function deleteJourney(id,dateHint='',timeHint=''){
 ...
 if(!d || !t){alert('No se pudo identificar la fecha y hora de la jornada.');return}
 if(!confirm(`¿Borrar definitivamente la jornada ${d} · ${t}?\n\nEsta acción eliminará el registro de Supabase y no se puede deshacer.`))return;
 try{
   // La tarjeta del Historial ya conoce fecha y hora de la jornada visible.
   // Usamos esos valores para consultar directamente a Supabase y obtener
   // el ID real. Así el borrado no depende del estado del arreglo rows.
   const found=await withTimeout(
     db.from('jornadas_trabajo')
       .select('id,fecha,hora_inicio')
       .eq('fecha',d)
       .gte('hora_inicio',t+':00')
       .lt('hora_inicio',t+':59')
       .order('created_at',{ascending:false})
   );
   if(found.error)throw found.error;
   if(!found.data || found.data.length===0){
     throw new Error('No existe en Supabase una jornada con la fecha y hora seleccionadas.');
   }
   const deleteId=found.data[0].id;

   const result=await withTimeout(
     db.from('jornadas_trabajo').delete().eq('id',deleteId)
   );
   if(result.error)throw result.error;

   // Verificación independiente del DELETE.
   const check=await withTimeout(
     db.from('jornadas_trabajo').select('id').eq('id',deleteId).maybeSingle()
   );
   if(check.error)throw check.error;
   if(check.data)throw new Error('Supabase no eliminó el registro. Revisa las políticas RLS de DELETE.');

   // Refrescar todo desde la base de datos.
   await load();
